...
# TCP buffers: min / default / max  (4KB / 1MB / 64MB)
...
sysctl -w vm.dirty_expire_centisecs=1500              >/dev/null
sysctl -w vm.dirty_writeback_centisecs=500            >/dev/null
sysctl -w vm.page-cluster=0                           >/dev/null
sysctl -w vm.min_free_kbytes=524288                   >/dev/null
sysctl -w vm.zone_reclaim_mode=0                      >/dev/null
sysctl -w vm.compaction_proactiveness=0               >/dev/null 2>&1 || true
...
sysctl -w kernel.sched_migration_cost_ns=250000       >/dev/null 2>&1 || true
...
###############################################################################
# 5b. AMD SOFTWARE STACK TUNING (Mesa / RADV / ACO / env vars)
###############################################################################
log "Tuning AMD software stack (Mesa/RADV/Vulkan)..."

AMD_ENV_FILE="/etc/environment.d/99-amd-performance.conf"
mkdir -p /etc/environment.d

cat > "$AMD_ENV_FILE" <<'AMDENV'
# ── AMD Mesa / RADV / Vulkan Performance ─────────────────

# Force RADV (AMD open-source Vulkan driver)
AMD_VULKAN_ICD=RADV

# Enable ACO shader compiler backend (faster compile, better perf)
RADV_PERFTEST=aco,gpl,nggc,sam

# Mesa threading — offload GL calls to a separate thread
mesa_glthread=true

# Disable VSync at driver level (let app/compositor handle it)
vblank_mode=0

# Shader disk cache — keep compiled shaders cached
MESA_SHADER_CACHE_DISABLE=false
MESA_SHADER_CACHE_MAX_SIZE=10G
RADV_FORCE_FAMILY=

# Force DCC (Delta Color Compression) — better bandwidth
RADV_DEBUG=nodcc=0

# Disable Mesa GL error checking (less overhead)
MESA_NO_ERROR=1

# Enable NGG (Next-Gen Geometry) culling on RDNA+
RADV_PERFTEST=nggc

# AMD GPU scheduler priority — high priority for graphics
RADV_TEX_ANISO=0

# Disable stall-inducing debug features
LIBGL_NO_DRAWARRAYS=1

# ── AMD CPU ──────────────────────────────────────────────

# Use performance-oriented malloc (if available)
# Uncomment if you have jemalloc or mimalloc installed:
# LD_PRELOAD=/usr/lib/libjemalloc.so
AMDENV

log "  → Written $AMD_ENV_FILE"
...
# Disable amd_pstate prefcore if we want all cores equal (optional — uncomment for all-core loads)
# if [[ -f /sys/devices/system/cpu/amd_pstate/prefcore ]]; then
#     echo 0 > /sys/devices/system/cpu/amd_pstate/prefcore 2>/dev/null || true
# fi
...
if [[ -d /sys/module/amdgpu/parameters ]]; then
    ...
    # Reduce GPU scheduling latency
    echo 0 > /sys/module/amdgpu/parameters/sched_jobs 2>/dev/null 2>&1 || true
...
# Disable mitigations message (if desired — uncomment for extra perf)
# Note: Only effective at boot via cmdline. Just informational here.
# mitigations=off retbleed=off spectre_v2=off spec_store_bypass_disable=off
